...
const PopupForm: React.FC<PopupProps> = ({ isOpen, onClose, onSubmit, email }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        await onSubmit({ firstName, lastName, waitlistRole, email : email === "" ? emailAddress : email });
        ...
        if (!firstName.trim() || !nameRegex.test(firstName.trim())) {
            ...
            setToast({ show: true, message: "Please enter a valid first name.", type: "error" });
            return;
        }

        if (!lastName.trim() || !nameRegex.test(lastName.trim())) {
            setLastNameValid(false);
            setToast({ show: true, message: "Please enter a valid last name.", type: "error" });
            return;
        }
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black/50 z-50" onClick={handleClose}>
        <div onClick={(e) => e.stopPropagation()} className="w-[300px] sm:w-[400px] md:w-[450px] bg-white rounded-xl shadow-lg p-6 relative transform animate-fadeInUp ">
            <button onClick={onClose} className="absolute top-6 right-6 text-xl text-gray-500 hover:text-gray-800 cursor-pointer">
            ✕
            </button>
            ...
            <h2 className="text-xl font-semibold mb-4">Complete Sign Up</h2>
             }
            ...
            <form onSubmit={handleSubmit} className="space-y-4 flex flex-col items-center justify-center ">
                ...
                {email === "" && (
                    <div className="relative w-full">
                        ...
                        className={`w-full xs:mb-3 px-4 py-3 sm:py-4 text-lg sm:text-xl bg-white rounded-2xl border border-gray-400 focus:ring-2 focus:ring-brand outline-none font-heading pr-12 ${emailValid === false ? "border-red-500" : "border-green-500"}`}
                        />
                ...
                <button
                    type="submit"
                    disabled={loading}
                    className={`text-white font-bold text-xl px-10 py-2.5 rounded-full shadow-md
