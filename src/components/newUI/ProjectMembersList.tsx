import { useEffect, useState } from "react";
import { CreateOutlined } from '@mui/icons-material';
import { useAuth } from "../../context/auth-context";

type User = {
  id: number;
  fullName: string;
  email: string;
  company?: string;
  discipline?: string;
  position?: string;
  status?: string;
  avatar?: string;
};

type ProjectMember = {
  id: number;
  projectId: number;
  userId: number;
  assignedAt: string;
  user: User;
};

interface ProjectMembersListProps {
  projectId: number;
}

const ProjectMembersList: React.FC<ProjectMembersListProps> = ({ projectId }) => {
    const { accessToken } = useAuth() ?? {};
    const [members, setMembers] = useState<ProjectMember[]>([]);
    const [selected, setSelected] = useState<number[]>([]);

    useEffect(() => {
      if (!projectId) return;
      fetch(`${import.meta.env.VITE_API_URL}/api/project-members/${projectId}`, {
        method: "GET",
        credentials: 'include'
      })
        .then(res => res.json())
        .then(async (data) => {
          // For each member, fetch user details
          const membersWithUser = await Promise.all(
            data.map(async (member: ProjectMember) => {
              try {
                const userRes = await fetch(`${import.meta.env.VITE_API_URL}/api/users/${member.userId}`, {
        method: "GET",
        credentials: 'include'
      });
                const userData = await userRes.json();
                return { ...member, user: userData };
              } catch {
                return { ...member, user: undefined };
              }
            })
          );
          setMembers(membersWithUser);
        })
        .catch(err => console.error("Failed to fetch members", err));
    }, [projectId]);


    const toggleSelect = (id: number) => {
      setSelected((prev: number[]) =>
        prev.includes(id) ? prev.filter((x: number) => x !== id) : [...prev, id]
      );
    };

    const toggleSelectAll = () => {
      if (selected.length === members.length) setSelected([]);
      else setSelected(members.map((m: ProjectMember) => m.id));
    };

  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
      <table className="w-full text-sm text-gray-700 text-center">
        <thead className="bg-gray-50 border-b border-gray-200 text-xl">
          <tr>
            <th className="p-4">
              <input
                type="checkbox"
                checked={selected.length === members.length}
                onChange={toggleSelectAll}
                className="w-4 h-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
              />
            </th>
            <th className="px-4 py-3 font-semibold text-gray-900 text-left">Name</th>
            <th className="px-4 py-3 font-semibold text-gray-900">Company</th>
            <th className="px-4 py-3 font-semibold text-gray-900">Discipline</th>
            <th className="px-4 py-3 font-semibold text-gray-900">Position</th>
            <th className="px-4 py-3 font-semibold text-gray-900">Account Status</th>
            <th className="px-4 py-3"></th>
          </tr>
        </thead>
        <tbody>
          {members.map((member: ProjectMember) => (
            <tr
              key={member.id}
              className="border-b border-gray-100 hover:bg-gray-50 transition-colors text-lg"
            >
              <td className="p-4">
                <input
                  type="checkbox"
                  checked={selected.includes(member.id)}
                  onChange={() => toggleSelect(member.id)}
                  className="w-4 h-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                />
              </td>
              <td className="px-4 py-3 flex items-center gap-3 text-left">
                <img
                  src={member.user?.avatar || `https://i.pravatar.cc/${Math.floor(Math.random() * (50 - 1 + 1)) + 1}`}
                  alt={member.user?.fullName || "Unknown User"}
                  className="w-10 h-10 rounded-full object-cover"
                />
                <div>
                  <p className="font-semibold text-gray-900">{member.user?.fullName || "Unknown User"}</p>
                  <p className="text-gray-500 text-sm">{member.user?.email || ""}</p>
                </div>
              </td>
              <td className="px-4 py-3">{member.user?.company || ""}</td>
              <td className="px-4 py-3">{member.user?.discipline || ""}</td>
              <td className="px-4 py-3">{member.user?.position || ""}</td>
              <td className="px-4 py-3 text-green-600 font-medium">{member.user?.status || ""}</td>
              <td className="px-4 py-3 text-gray-400 hover:text-gray-600 cursor-pointer">
                <CreateOutlined />
              </td>
            </tr>
          ))}
        </tbody>
      </table>
    </div>
  )
}

export default ProjectMembersList
